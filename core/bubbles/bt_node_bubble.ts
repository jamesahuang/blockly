 ...
 */
export class BtNodeBubble implements IBubble {
   ...
   * @param pos The textinput pos = translate value
   */
  constructor(
    ...
    this.tail = dom.createSvgElement(Svg.PATH, {
      'marker-end': `url(#${
        this.workspace.getRenderer().getConstants().lineArrowMarkerId
      })`,
      'stroke-width':'2',
      fill: 'none',
      stroke: 'black',
    ...
    this.background = dom.createSvgElement(
      Svg.RECT,
      {
        ...
        'x': 0,
        'y': 0,
        'rx': BtNodeBubble.BORDER_WIDTH,
        'ry': BtNodeBubble.BORDER_WIDTH,
      },
      embossGroup,
    );
  ...
  protected setSize(size: Size, relayout = false) {
    size.width = Math.max(size.width, BtNodeBubble.MIN_SIZE);
    size.height = Math.max(size.height, BtNodeBubble.MIN_SIZE);
    this.size = size;
  ...
  renderTail() {
    ...
    if (relBubbleX === relAnchorX && relBubbleY === relAnchorY) {
      ...
      let thickness =
        (this.size.width + this.size.height) / BtNodeBubble.TAIL_THICKNESS;
      thickness = Math.min(thickness, this.size.width, this.size.height) / 4;

      // Back the tip of the tail off of the anchor.
      const backoffRatio = 1 - BtNodeBubble.ANCHOR_RADIUS / hypotenuse;
      ...
      const radians = math.toRadians(
        this.workspace.RTL ? -BtNodeBubble.TAIL_ANGLE : BtNodeBubble.TAIL_ANGLE,
      );
      let swirlAngle = angle + radians;
      if (swirlAngle > Math.PI * 2) {
        swirlAngle -= Math.PI * 2;
      }
      const swirlRise = (Math.sin(swirlAngle) * hypotenuse) / BtNodeBubble.TAIL_BEND;
      const swirlRun = (Math.cos(swirlAngle) * hypotenuse) / BtNodeBubble.TAIL_BEND;
